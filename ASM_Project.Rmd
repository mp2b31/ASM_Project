...
AR(2) SAR(4)
d=0, D=0, test if intercept is significant 
...
intercept t ratio is lower than 2 so, is not significant
...
Also the AIC is lower for the model with regular an seasonal difference.
All the weights are significant.
...
mod2 have lower AIC so we take the second model
...
```{r}
(mod2TD=arima(lnserie,order=c(4,1,0),seasonal=list(order=c(4,1,0),period=12),xreg=wTradDays))
abs(mod2TD$coef/sqrt(diag(mod2TD$var.coef)))
```

```{r}
(mod2Ea=arima(lnserie,order=c(4,1,0),seasonal=list(order=c(4,1,0),period=12),xreg=wEast))
abs(mod2Ea$coef/sqrt(diag(mod2Ea$var.coef)))
```

```{r}
(mod2EC=arima(lnserie,order=c(4,1,0),seasonal=list(order=c(4,1,0),period=12),xreg=data.frame(wTradDays,wEast)))
...
mod.atip=outdetec(mod2EC,dif=c(1,12),crit=3.0,LS=T)
mod.atip$sigma
```
